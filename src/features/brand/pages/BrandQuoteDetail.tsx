...
const statusLabels: Record<string, string> = {
  draft: "Draft", sent: "Sent", pending: "Pending", in_negotiation: "In Negotiation",
  accepted: "Accepted", declined: "Declined", expired: "Expired",
  awaiting_payment: "Awaiting Payment", payment_overdue: "Payment Overdue",
...
export function BrandQuoteDetail() {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="rounded-full border border-border bg-muted/50 px-2.5 py-0.5 text-xs font-medium text-muted-foreground">
            {quote.quoteType === "framework_contract" ? "Framework Contract" : "One-time"}
          </span>
          <span className="rounded-full border border-border bg-muted/50 px-2.5 py-0.5 text-xs font-medium text-muted-foreground">
            {quote.initiator === "creator" ? "From creator" : "Your order"}
          </span>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  {creator.paymentDetails.method} · {creator.paymentDetails.info}
                </p>
              </div>
            </div>
          </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {oneTimeOffers.map((offer) => {
                      ...
                      const invSc = inv ? invoiceStatusColors[inv.status] : null;
                      return (
                        <tr key={offer!.id} className="border-b border-border last:border-0">
                          <td className="px-5 py-3 font-medium">{offer!.title}</td>
                          ...
                          <td className="px-5 py-3">
                            {inv ? (
                              <span className="text-xs font-medium text-primary">{inv.id}</span>
                            ) : (
                            ...
                            {inv && invSc ? (
                              <div className="flex items-center gap-2">
                                ...
                                {(inv.status === "sent" || inv.status === "overdue") && (
                                  <button className="inline-flex items-center gap-1 rounded-md bg-primary px-2 py-0.5 text-[10px] font-medium text-white hover:bg-primary/90">
                    ...
                    <div key={offer!.id} className="p-5 space-y-3">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground capitalize">
                          {offer!.category.replace(/_/g, " ")} ·{" "}
                          {formatCurrency(offer!.price, offer!.currency)}/month
                        </p>
                      </div>
                      {tasks.length > 0 && (
                        <div className="overflow-x-auto rounded-lg border border-border">
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {tasks.map((task) => {
                                const rsc = recurringStatusColors[task.status] ?? recurringStatusColors.upcoming;
                                return (
                                  <tr key={task.id} className="border-b border-border last:border-0">
                                    <td className="px-4 py-2.5 font-medium">{task.period}</td>
            ...
            <div className="rounded-xl border border-border bg-card">
                ...
                {quoteNegotiations.length > 3 && (
                  ...
                  >
                    {chatExpanded ? (
                      ...
                      <>Show all ({quoteNegotiations.length}) <ChevronDown className="h-3 w-3" /></>
                    )}
                  </button>
                )}
              </div>
              <div className="p-5 space-y-3">
                {(chatExpanded
                  ...
                  if (msg.type === "system") {
                    return (
                      <div key={msg.id} className="text-center text-xs text-muted-foreground italic py-1">
                        {msg.message}
                        <span className="ml-2 opacity-60">{formatDate(msg.timestamp)}</span>
                      </div>
                    );
                  }
                        ...
                        <div className="max-w-[80%] rounded-lg border border-amber-200 bg-amber-50 p-3 space-y-1.5">
                          ...
                          {msg.revisions?.map((rev, i) => (
                            <div key={i} className="flex items-center gap-2 text-xs">
                              <span className="font-medium text-muted-foreground">{rev.field}:</span>
                              <span className="line-through text-red-600">{rev.oldValue}</span>
                              <ArrowRight className="h-3 w-3 text-muted-foreground" />
                              <span className="text-emerald-700 font-medium">{rev.newValue}</span>
                            </div>
                          ))}
                  ...
                  // Client messages right-aligned, creator left-aligned (opposite of creator view)
                    ...
                    >
                      <div
                        className={cn(
                          ...
                          isClient ? "bg-primary text-white" : "bg-muted/60",
                        )}
                      >
                        <p className="text-xs font-medium opacity-80">{msg.authorName}</p>
                        <p className="text-sm">{msg.message}</p>
                        <p className={cn("text-[10px]", isClient ? "text-white/60" : "text-muted-foreground")}>
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-border">
                  ...
                  />
                  <button disabled className="rounded-lg bg-primary/50 p-2 text-white disabled:cursor-not-allowed">
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Negotiation Type</p>
                <p className="text-sm font-medium">
                  {quote.negotiationType === "with_negotiation" ? "With Negotiation" : "Direct Invoice"}
                </p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Late Payment</p>
                <span
                  className={cn(
        ...
        <div className="lg:sticky lg:top-20 lg:self-start space-y-4">
          ...
          <ClientActionButtons status={localStatus} onStatusChange={setLocalStatus} />
        </div>
      </div>
    </div>
  );
          ...
          return (
            <div key={step.key + i} className="flex gap-3">
              <div className="flex flex-col items-center">
                <div
                  className={cn(
                    ...
                    step.status === "active" && "border-primary bg-primary text-white animate-pulse",
                    step.status === "upcoming" && "border-gray-300 bg-white",
                    step.status === "skipped" && "border-gray-200 bg-gray-100",
                  )}
                >
                  {step.status === "completed" && <Check className="h-3.5 w-3.5" />}
                  {step.status === "active" && <div className="h-2 w-2 rounded-full bg-white" />}
                </div>
                {!isLast && (
                  <div
                    className={cn(
              ...
              <div className="pb-4">
                ...
                {step.completedAt && (
                  <p className="text-xs text-muted-foreground">{formatDate(step.completedAt)}</p>
                )}
              </div>
            </div>
          );
...
function ClientActionButtons({
  ...
  const terminal = new Set<QuoteStatus>(["completed", "cancelled", "declined", "expired"]);
    ...
    <div className="space-y-2">
      {(status === "sent" || status === "pending") && (
        <>
          ...
          <button
            onClick={() => onStatusChange("in_negotiation")}
            className="w-full rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-700 transition-colors inline-flex items-center justify-center gap-2"
          >
          ...
          <button
            ...
            className="w-full rounded-lg border border-red-300 bg-white px-4 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50 transition-colors inline-flex items-center justify-center gap-2"
          >
      ...
      {status === "in_progress" && (
        <div className="w-full rounded-lg border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm text-center text-blue-700 font-medium">
          <Clock className="inline h-4 w-4 mr-1.5" />
          Awaiting delivery…
        </div>
      )}
      ...
      {!terminal.has(status) && status !== "in_negotiation" && status !== "accepted" && status !== "in_progress" && (
        <button
          onClick={() => onStatusChange("declined")}
